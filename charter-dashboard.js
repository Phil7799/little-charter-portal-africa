// Charter Dashboard JavaScript - v6.0
// Updated: busbuddy net revenue = 20%
let charterCharts = {};
...
function updateCharterStats(dashboardData, year) {
    ...
    // Net revenue = 20% of total revenue (updated from 18%)
...
function updateCharterMonthlyTargetChart(dashboardData, months, fullMonths, year) {
    ...
    charterCharts.monthlyTargetChart = new Chart(ctx, {
        type: 'bar',
        data: { labels: months, datasets: [
            { label: 'Actual 2026', data: actual, backgroundColor: 'rgba(26,115,232,0.75)', borderWidth: 1 },
            { label: 'Target 2026', data: target, backgroundColor: 'rgba(251,188,4,0.75)', borderWidth: 1 }
        ]},
...
function updateCharterAssociatePerformanceChart(dashboardData, year) {
    ...
    const perf2025 = associates.map(a => (dashboardData.charter.byAssociate[a].total2025 || 0) / 1e6);
    const perf2026 = associates.map(a => (dashboardData.charter.byAssociate[a].total2026 || 0) / 1e6);
    const targets = associates.map(a => (dashboardData.charter.byAssociate[a].target2026 || 0) / 1e6);
    charterCharts.associatePerformanceChart = new Chart(ctx, {
        type: 'bar',
        data: { labels: associates.map(a => a.split('@')[0]), datasets: [
            { label: '2025 Revenue', data: perf2025, backgroundColor: 'rgba(26,115,232,0.6)', borderWidth: 1 },
            { label: '2026 Revenue', data: perf2026, backgroundColor: 'rgba(52,168,83,0.75)', borderWidth: 1 },
            { label: '2026 Target', data: targets, backgroundColor: 'rgba(251,188,4,0.75)', borderWidth: 1 }
        ]},
...
function updateCharterTable(dashboardData, year) {
    ...
    monthsToShow.forEach(month => {
        ...
        associates.forEach(assoc => {
            ...
            const net = d.total2026 * 0.20; // Updated to 20%
            addTableRow(tbody, [
                month, assoc.split('@')[0],
